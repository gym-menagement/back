package rest


import (
	"gym/controllers"
	"gym/models"

    "strings"
)

type PushtokenController struct {
	controllers.Controller
}

func (c *PushtokenController) Read(id int64) {
    
    
	conn := c.NewConnection()

	manager := models.NewPushtokenManager(conn)
	item := manager.Get(id)

    
    
    c.Set("item", item)
}

func (c *PushtokenController) Index(page int, pagesize int) {
    
    
	conn := c.NewConnection()

	manager := models.NewPushtokenManager(conn)

    var args []interface{}
    
    _user := c.Geti64("user")
    if _user != 0 {
        args = append(args, models.Where{Column:"user", Value:_user, Compare:"="})    
    }
    _token := c.Get("token")
    if _token != "" {
        args = append(args, models.Where{Column:"token", Value:_token, Compare:"like"})
    }
    _devicetype := c.Get("devicetype")
    if _devicetype != "" {
        args = append(args, models.Where{Column:"devicetype", Value:_devicetype, Compare:"like"})
    }
    _deviceid := c.Get("deviceid")
    if _deviceid != "" {
        args = append(args, models.Where{Column:"deviceid", Value:_deviceid, Compare:"like"})
    }
    _appversion := c.Get("appversion")
    if _appversion != "" {
        args = append(args, models.Where{Column:"appversion", Value:_appversion, Compare:"like"})
    }
    _isactive := c.Geti("isactive")
    if _isactive != 0 {
        args = append(args, models.Where{Column:"isactive", Value:_isactive, Compare:"="})    
    }
    _startcreateddate := c.Get("startcreateddate")
    _endcreateddate := c.Get("endcreateddate")
    if _startcreateddate != "" && _endcreateddate != "" {        
        var v [2]string
        v[0] = _startcreateddate
        v[1] = _endcreateddate  
        args = append(args, models.Where{Column:"createddate", Value:v, Compare:"between"})    
    } else if  _startcreateddate != "" {          
        args = append(args, models.Where{Column:"createddate", Value:_startcreateddate, Compare:">="})
    } else if  _endcreateddate != "" {          
        args = append(args, models.Where{Column:"createddate", Value:_endcreateddate, Compare:"<="})            
    }
    _startupdateddate := c.Get("startupdateddate")
    _endupdateddate := c.Get("endupdateddate")
    if _startupdateddate != "" && _endupdateddate != "" {        
        var v [2]string
        v[0] = _startupdateddate
        v[1] = _endupdateddate  
        args = append(args, models.Where{Column:"updateddate", Value:v, Compare:"between"})    
    } else if  _startupdateddate != "" {          
        args = append(args, models.Where{Column:"updateddate", Value:_startupdateddate, Compare:">="})
    } else if  _endupdateddate != "" {          
        args = append(args, models.Where{Column:"updateddate", Value:_endupdateddate, Compare:"<="})            
    }
    _startdate := c.Get("startdate")
    _enddate := c.Get("enddate")
    if _startdate != "" && _enddate != "" {        
        var v [2]string
        v[0] = _startdate
        v[1] = _enddate  
        args = append(args, models.Where{Column:"date", Value:v, Compare:"between"})    
    } else if  _startdate != "" {          
        args = append(args, models.Where{Column:"date", Value:_startdate, Compare:">="})
    } else if  _enddate != "" {          
        args = append(args, models.Where{Column:"date", Value:_enddate, Compare:"<="})            
    }
    

    
    
    if page != 0 && pagesize != 0 {
        args = append(args, models.Paging(page, pagesize))
    }
    
    orderby := c.Get("orderby")
    if orderby == "" {
        if page != 0 && pagesize != 0 {
            orderby = "id desc"
            args = append(args, models.Ordering(orderby))
        }
    } else {
        orderbys := strings.Split(orderby, ",")

        str := ""
        for i, v := range orderbys {
            if i == 0 {
                str += v
            } else {
                if strings.Contains(v, "_") {                   
                    str += ", " + strings.Trim(v, " ")
                } else {
                    str += ", pt_" + strings.Trim(v, " ")                
                }
            }
        }
        
        args = append(args, models.Ordering(str))
    }
    
	items := manager.Find(args)
	c.Set("items", items)

    if page == 1 {
       total := manager.Count(args)
	   c.Set("total", total)
    }
}

func (c *PushtokenController) Count() {
    
    
	conn := c.NewConnection()

	manager := models.NewPushtokenManager(conn)

    var args []interface{}
    
    _user := c.Geti64("user")
    if _user != 0 {
        args = append(args, models.Where{Column:"user", Value:_user, Compare:"="})    
    }
    _token := c.Get("token")
    if _token != "" {
        args = append(args, models.Where{Column:"token", Value:_token, Compare:"like"})
        
    }
    _devicetype := c.Get("devicetype")
    if _devicetype != "" {
        args = append(args, models.Where{Column:"devicetype", Value:_devicetype, Compare:"like"})
        
    }
    _deviceid := c.Get("deviceid")
    if _deviceid != "" {
        args = append(args, models.Where{Column:"deviceid", Value:_deviceid, Compare:"like"})
        
    }
    _appversion := c.Get("appversion")
    if _appversion != "" {
        args = append(args, models.Where{Column:"appversion", Value:_appversion, Compare:"like"})
        
    }
    _isactive := c.Geti("isactive")
    if _isactive != 0 {
        args = append(args, models.Where{Column:"isactive", Value:_isactive, Compare:"="})    
    }
    _startcreateddate := c.Get("startcreateddate")
    _endcreateddate := c.Get("endcreateddate")

    if _startcreateddate != "" && _endcreateddate != "" {        
        var v [2]string
        v[0] = _startcreateddate
        v[1] = _endcreateddate  
        args = append(args, models.Where{Column:"createddate", Value:v, Compare:"between"})    
    } else if  _startcreateddate != "" {          
        args = append(args, models.Where{Column:"createddate", Value:_startcreateddate, Compare:">="})
    } else if  _endcreateddate != "" {          
        args = append(args, models.Where{Column:"createddate", Value:_endcreateddate, Compare:"<="})            
    }
    _startupdateddate := c.Get("startupdateddate")
    _endupdateddate := c.Get("endupdateddate")

    if _startupdateddate != "" && _endupdateddate != "" {        
        var v [2]string
        v[0] = _startupdateddate
        v[1] = _endupdateddate  
        args = append(args, models.Where{Column:"updateddate", Value:v, Compare:"between"})    
    } else if  _startupdateddate != "" {          
        args = append(args, models.Where{Column:"updateddate", Value:_startupdateddate, Compare:">="})
    } else if  _endupdateddate != "" {          
        args = append(args, models.Where{Column:"updateddate", Value:_endupdateddate, Compare:"<="})            
    }
    _startdate := c.Get("startdate")
    _enddate := c.Get("enddate")

    if _startdate != "" && _enddate != "" {        
        var v [2]string
        v[0] = _startdate
        v[1] = _enddate  
        args = append(args, models.Where{Column:"date", Value:v, Compare:"between"})    
    } else if  _startdate != "" {          
        args = append(args, models.Where{Column:"date", Value:_startdate, Compare:">="})
    } else if  _enddate != "" {          
        args = append(args, models.Where{Column:"date", Value:_enddate, Compare:"<="})            
    }
    
    
    
    
    total := manager.Count(args)
	c.Set("total", total)
}

func (c *PushtokenController) Insert(item *models.Pushtoken) {
    
    
	conn := c.NewConnection()
    
	manager := models.NewPushtokenManager(conn)
	err := manager.Insert(item)
    if err != nil {
        c.Set("code", "error")    
        c.Set("error", err)
        return
    }

    id := manager.GetIdentity()
    c.Result["id"] = id
    item.Id = id
}

func (c *PushtokenController) Insertbatch(item *[]models.Pushtoken) {  
    if item == nil || len(*item) == 0 {
        return
    }

    rows := len(*item)
    
    
    
	conn := c.NewConnection()
    
	manager := models.NewPushtokenManager(conn)

    for i := 0; i < rows; i++ {
	    err := manager.Insert(&((*item)[i]))
        if err != nil {
            c.Set("code", "error")    
            c.Set("error", err)
            return
        }
    }
}

func (c *PushtokenController) Update(item *models.Pushtoken) {
    
    
	conn := c.NewConnection()

	manager := models.NewPushtokenManager(conn)
    err := manager.Update(item)
    if err != nil {
        c.Set("code", "error")    
        c.Set("error", err)
        return
    }
}

func (c *PushtokenController) Delete(item *models.Pushtoken) {
    
    
    conn := c.NewConnection()

	manager := models.NewPushtokenManager(conn)

    
	err := manager.Delete(item.Id)
    if err != nil {
        c.Set("code", "error")    
        c.Set("error", err)
    }
}

func (c *PushtokenController) Deletebatch(item *[]models.Pushtoken) {
    
    
    conn := c.NewConnection()

	manager := models.NewPushtokenManager(conn)

    for _, v := range *item {
        
    
	    err := manager.Delete(v.Id)
        if err != nil {
            c.Set("code", "error")    
            c.Set("error", err)
            return
        }
    }
}


