...
func NewRoleManager(conn *Connection) *RoleManager {
...
func (p *RoleManager) SetCountQuery(query string) {
    p.CountQuery = query
}

func (p *RoleManager) SetSelectQuery(query string) {
    p.SelectQuery = query
}
...
func (p *RoleManager) GetQuery() string {
    if p.SelectQuery != "" {
    ...
    ret.WriteString("select r_id, r_gym, r_roleid, r_name, r_date, g_id, g_name, g_address, g_tel, g_user, g_date from role_tb, gym_tb")
...
func (p *RoleManager) GetQuerySelect() string {
    if p.CountQuery != "" {
        return p.CountQuery    
    }
    ...
    ret.WriteString("select count(*) from role_tb")
...
func (p *RoleManager) GetQueryGroup(name string) string {
    if p.SelectQuery != "" {
        return p.SelectQuery    
    }
    ...
    ret.WriteString(", count(*) from role_tb ")
...
func (p *RoleManager) DeleteAll() error {
    if !p.Conn.IsConnect() {
        return errors.New("Connection Error")
    }

    query := "delete from role_tb"
    _, err := p.Exec(query)
...
func (p *RoleManager) MakeQuery(initQuery string , postQuery string, initParams []interface{}, args []interface{}) (string, []interface{}) {
...
func (p *RoleManager) DeleteWhere(args []interface{}) error {
    if !p.Conn.IsConnect() {
        return errors.New("Connection Error")
    }

    query, params := p.MakeQuery("delete from role_tb where 1=1", "", nil, args)
...
func (p *RoleManager) UpdateWhere(columns []role.Params, args []interface{}) error {
    if !p.Conn.IsConnect() {
        return errors.New("Connection Error")
    }
    ...
    initQuery.WriteString("update role_tb set ")
    for i, v := range columns {
        if i > 0 {
            initQuery.WriteString(", ")
        }

        if v.Column == role.ColumnId {
        ...
        initParams = append(initParams, v.Value)
        } else if v.Column == role.ColumnDate {
...
func (p *RoleManager) UpdateGym(value int64, id int64) error {
    if !p.Conn.IsConnect() {
        return errors.New("Connection Error")
    }

	query := "update role_tb set r_gym = ? where r_id = ?"
...
func (p *RoleManager) UpdateRoleid(value role.Roleid, id int64) error {
    if !p.Conn.IsConnect() {
        return errors.New("Connection Error")
    }
...
func (p *RoleManager) UpdateName(value string, id int64) error {
    if !p.Conn.IsConnect() {
        return errors.New("Connection Error")
    }

	query := "update role_tb set r_name = ? where r_id = ?"
	_, err := p.Exec(query, value, id)
...
func (p *RoleManager) UpdateDate(value string, id int64) error {
    if !p.Conn.IsConnect() {
        return errors.New("Connection Error")
    }

	query := "update role_tb set r_date = ? where r_id = ?"
	_, err := p.Exec(query, value, id)
...
func (p *Role) InitExtra() {
    p.Extra = map[string]interface{}{
            "roleid":     role.GetRoleid(p.Roleid),
...
func (p *RoleManager) ReadRow(rows *sql.Rows) *Role {
    ...
    if rows.Next() {
        err = rows.Scan(&item.Id, &item.Gym, &item.Roleid, &item.Name, &item.Date, &_gym.Id, &_gym.Name, &_gym.Address, &_gym.Tel, &_gym.User, &_gym.Date)
...
func (p *RoleManager) GetWhere(args []interface{}) *Role {
    items := p.Find(args)
    if len(items) == 0 {
        return nil
    }
...
func (p *RoleManager) FindAll() []Role {
    return p.Find(nil)
}
...
func (p *RoleManager) GroupBy(name string, args []interface{}) []Groupby {
    if !p.Conn.IsConnect() {
        var items []Groupby
        return items
    }
