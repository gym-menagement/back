...
func SetupOrderRoutes(group fiber.Router) {
	...
	group.Get("/order/:id", func(c *fiber.Ctx) error {
	...
	group.Post("/order/batch", func(c *fiber.Ctx) error {
		var items_ *[]models.Order
		items__ref := &items_
	...
	group.Put("/order", func(c *fiber.Ctx) error {
		item_ := &models.Order{}
		err := c.BodyParser(item_)
		if err != nil {
		    log.Error().Msg(err.Error())
	...
	group.Delete("/order", func(c *fiber.Ctx) error {
		item_ := &models.Order{}
		err := c.BodyParser(item_)
		if err != nil {
		    log.Error().Msg(err.Error())
	...
	group.Delete("/order/batch", func(c *fiber.Ctx) error {
		item_ := &[]models.Order{}
		err := c.BodyParser(item_)
		if err != nil {
		    log.Error().Msg(err.Error())
